...
interface Props { onLogin: (email: string, pass: string) => Promise<void> }
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:48, background:'#FAFBF9' }}>
        <div style={{ width:'100%', maxWidth:380, animation:'fadeUp .5s ease both' }}>
          ...
          <form onSubmit={submit}>
            ...
            <button type="submit" disabled={loading} style={{ width:'100%', background:'var(--c-ink)', color:'#fff', borderRadius:12, padding:'14px', fontWeight:700, fontSize:15.5, display:'flex', alignItems:'center', justifyContent:'center', gap:9, transition:'transform .15s, opacity .15s', opacity: loading ? .7 : 1 }}
              onMouseEnter={e => { if (!loading) (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)' }}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.transform = ''}
            >
              {loading ? <><Spinner size={16} color="#fff" /> Signing in…</> : <>Sign in <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--c-accent)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 6l6 6-6 6"/></svg></>}
            </button>
          </form>

          <p style={{ marginTop:24, textAlign:'center', fontSize:13, color:'var(--c-faint)' }}>
            Don&apos;t have an account?{' '}
            <span style={{ color:'var(--c-accent-dark)', fontWeight:600, cursor:'pointer' }}>Contact your admin</span>
          </p>
        </div>
      </div>
    </div>
